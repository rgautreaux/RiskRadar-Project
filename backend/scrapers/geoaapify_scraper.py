"""Geomapify - provides adress search, routes, maps, place information, and locational analysis

API site: https://www.geoapify.com/products-and-services/
...
class GeomapifyScraper(BaseScraper):
    ...
    def fetch_raw_data(self) -> list[dict]:
        ...
        if not settings.GEOMAPIFY_API_KEY:
            return []

        url = "https://api.geoapify.com/v1/places/by-bbox"
        params = {
            "format": "application/json",
            "bbox": f"{settings.DEFAULT_LON_MIN},{settings.DEFAULT_LAT_MIN},{settings.DEFAULT_LON_MAX},{settings.DEFAULT_LAT_MAX}",
            "radius": 25,
            "api_key": settings.GEOMAPIFY_API_KEY,
        }
