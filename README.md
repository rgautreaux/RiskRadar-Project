# RiskRadar ΓÇö Group 6 Senior Project
...
|------|---------|--------------|----------------|
| **Python** | 3.10+ | `py --version` | [python.org/downloads](https://www.python.org/downloads/) ΓÇö check "Add to PATH" during install |
| **Node.js** | 18+ | `node --version` | [nodejs.org](https://nodejs.org/) ΓÇö LTS version recommended |
...
```env
# REQUIRED ΓÇö generate with: py -c "import secrets; print(secrets.token_hex(32))"
...
# OPTIONAL ΓÇö for air quality data (free key from https://docs.airnowapi.org/account/request/)
...
| Scenario | What to do |
|----------|-----------|
| Local dev (default) | Do nothing ΓÇö `backend/riskradar.db` is created automatically |
...
1. Open your `.env` file (in the project root ΓÇö same folder as `backend/`)
2. Make sure these lines are **not** set to placeholder text ΓÇö either comment them out or leave them out entirely:
   ```env
   # DATABASE_URL=          ΓåÉ leave commented out for SQLite
   # DB_PATH=               ΓåÉ leave commented out to use backend/riskradar.db
   ```
...
> **Common mistake:** If you copy `.env.example` to `.env` without editing it, the placeholder values like `your-default-latitude-here` will cause startup errors. Replace all placeholders with real values or remove those lines entirely ΓÇö the settings have working defaults.

**If you ever need to reset the database:**
```bash
# Delete the database file and restart the server ΓÇö it will be recreated fresh
...
Verify it works by opening http://localhost:8000/docs in your browser ΓÇö you should see the Swagger API docs.
...
```
Team6Project/
Γö£ΓöÇΓöÇ .env.example              # Template for environment variables
Γö£ΓöÇΓöÇ .env                      # Your local config (not committed)
Γö£ΓöÇΓöÇ backend/                  # Python FastAPI server

Γöé   Γö£ΓöÇΓöÇ main.py               # App entry point
Γöé   Γö£ΓöÇΓöÇ requirements.txt      # Python dependencies
Γöé   Γö£ΓöÇΓöÇ riskradar.db          # SQLite database (auto-created)
Γöé   Γöé
Γöé   Γö£ΓöÇΓöÇ api/                  # REST API endpoints
Γöé   Γöé   Γö£ΓöÇΓöÇ router.py         # Mounts all routes under /api/v1
Γöé   Γöé   Γö£ΓöÇΓöÇ alerts.py         # GET /alerts, /alerts/stats
Γöé   Γöé   Γö£ΓöÇΓöÇ summaries.py      # GET/POST /summaries
Γöé   Γöé   Γö£ΓöÇΓöÇ users.py          # POST /register, /login + user profile
Γöé   Γöé   Γö£ΓöÇΓöÇ location.py       # GET /location/alerts ΓÇö on-demand fetch by zip code
Γöé   Γöé   ΓööΓöÇΓöÇ system.py         # GET /health, POST /scrape/trigger
Γöé   Γöé
Γöé   Γö£ΓöÇΓöÇ auth/security.py      # Password hashing (bcrypt) + JWT tokens
Γöé   Γö£ΓöÇΓöÇ config/settings.py    # All app settings loaded from .env
Γöé   Γö£ΓöÇΓöÇ config/sources.yaml   # Config-driven scraper definitions
Γöé   Γöé
Γöé   Γö£ΓöÇΓöÇ db/                   # Database layer
Γöé   Γöé   Γö£ΓöÇΓöÇ database.py       # SQLAlchemy engine + session
Γöé   Γöé   Γö£ΓöÇΓöÇ models.py         # Alert, Summary, User, ScrapeLog tables
Γöé   Γöé   ΓööΓöÇΓöÇ init_db.py        # Creates tables on startup
Γöé   Γöé
Γöé   Γö£ΓöÇΓöÇ scrapers/             # Data collection from external APIs
Γöé   Γöé   Γö£ΓöÇΓöÇ nws_scraper.py    # NOAA weather alerts
Γöé   Γöé   Γö£ΓöÇΓöÇ airnow_scraper.py # EPA air quality
Γöé   Γöé   Γö£ΓöÇΓöÇ epa_scraper.py    # EPA toxic releases
Γöé   Γöé   Γö£ΓöÇΓöÇ firms_scraper.py  # NASA wildfire data
Γöé   Γöé   ΓööΓöÇΓöÇ scheduler.py      # Runs scrapers every 30 minutes
Γöé   Γöé
Γöé   Γö£ΓöÇΓöÇ llm/summarizer.py     # AI-powered daily digest generation
Γöé   ΓööΓöÇΓöÇ tests/                # pytest test suite
Γöé
ΓööΓöÇΓöÇ frontend/
    ΓööΓöÇΓöÇ RiskRadar/            # React Native (Expo) mobile app
        Γö£ΓöÇΓöÇ package.json      # Node.js dependencies
        Γö£ΓöÇΓöÇ app/              # Screens (Expo Router file-based routing)
        Γöé   Γö£ΓöÇΓöÇ _layout.tsx   # Root layout
        Γöé   Γö£ΓöÇΓöÇ login.tsx     # Login screen
        Γöé   Γö£ΓöÇΓöÇ register.tsx  # Registration screen
        Γöé   ΓööΓöÇΓöÇ main/         # Authenticated screens
        Γöé       Γö£ΓöÇΓöÇ home.tsx          # Home ΓÇö search by zip code
        Γöé       Γö£ΓöÇΓöÇ weather-report.tsx # Weather + alerts for a location
        Γöé       Γö£ΓöÇΓöÇ settings.tsx      # User preferences
        Γöé       ΓööΓöÇΓöÇ _layout.tsx       # Tab navigation
        Γö£ΓöÇΓöÇ utils/api.ts      # API client (auto-adds JWT token)
        Γö£ΓöÇΓöÇ constants/api.ts  # Backend URL config (auto-detects IP)
        ΓööΓöÇΓöÇ contexts/auth-context.tsx  # Login/logout state management
```
...
Γöé   React Native App       Γöé
Γöé   (Expo on phone/web)    Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
            Γöé  HTTP requests to http://<your-ip>:8000/api/v1/...
            Γöé  JWT token in Authorization header
            Γöé
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé   FastAPI Backend        Γöé
Γöé   (Python on your PC)    Γöé
Γö£ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöñ
Γöé /api/v1/users/login      Γöé ΓåÆ returns JWT token
Γöé /api/v1/users/register   Γöé ΓåÆ creates account
Γöé /api/v1/alerts           Γöé ΓåÆ list all alerts from DB
Γöé /api/v1/location/alerts  Γöé ΓåÆ fetch fresh alerts for a zip code
Γöé /api/v1/summaries/latest Γöé ΓåÆ latest AI summary
Γöé /api/v1/health           Γöé ΓåÆ server status
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
            Γöé
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé   SQLite Database        Γöé
Γöé   (riskradar.db)         Γöé
...
| File | What it does |
|------|-------------|
| `frontend/RiskRadar/constants/api.ts` | Builds the backend URL ΓÇö auto-detects your PC's IP from Expo |
| `frontend/RiskRadar/utils/api.ts` | `apiFetch()` helper ΓÇö adds JWT token to every request |
...
- Apr 10 update: RebeccaΓÇÖs code-side implementation work is complete; the remaining dependency is backend/security lead sign-off, and the documentation bundle now reflects that boundary explicitly
...
- AUTHORS.md updated with each memberΓÇÖs contributions and roles in correct chronological order.
...
- The Apr 10 documentation pass preserved the same audit trail while making RebeccaΓÇÖs remaining approval gate explicit.
...
| POST | `/api/v1/users/login` | Login ΓåÆ returns JWT token |
...
### Location (Public ΓÇö on-demand fetch)
...
### Users (Protected ΓÇö requires JWT)
...
Expected: `87 passed`. Tests use an in-memory database and mock all external APIs ΓÇö no API keys needed.
...
| Backend says `402 Insufficient Balance` | Your LLM API key (DeepSeek) has no credits ΓÇö add funds or skip summary generation |
...
| `no such table: users` on startup | Your `.env` has `DB_PATH=` set to empty or a placeholder ΓÇö remove those lines so SQLite uses its default path (`backend/riskradar.db`) |
| `ValidationError` for `DEFAULT_LAT`, `ACCESS_TOKEN_EXPIRE_MINUTES` | `.env` still has placeholder text ΓÇö replace with real values or remove those lines |
